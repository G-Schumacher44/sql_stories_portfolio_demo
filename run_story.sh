...
# ==============================================================================
# run_story.sh - SQL Story Data Pipeline Orchestrator (Portfolio Demo)
#
...
# --- Argument Parsing ---
STORY_NAME=$1
# Use the second argument for DB name, or default to the portfolio version.
DB_NAME=${2:-"ecom_retailer.db"}

# --- Pre-flight Checks ---
if [ -z "$STORY_NAME" ]; then
  echo -e "${C_RED}❌ Error: No story name provided.${C_RESET}"
  echo "   Usage: $0 <story_name> [db_name]"
  echo "   Example: $0 story_01_inventory_audit"
  exit 1
fi
...
if [ ! -f "secrets.yaml" ] || [ ! -f "stories_config.yaml" ]; then
    ...
    echo "   Please ensure you have created 'secrets.yaml' and 'stories_config.yaml'."
...
BUILD_SCRIPTS=$(find "$SQL_SESSIONS_DIR" -maxdepth 1 -name "build_*.sql" 2>/dev/null | sort)
...
PYTHON_UPLOADER_SCRIPT="scripts/g_drive_uploader.py" # Note: This repo uses a different uploader script name

if [ -f "$PYTHON_UPLOADER_SCRIPT" ]; then
    # Execute the python script and check its exit code.
...
CLEANUP_SCRIPTS=$(find "$SQL_SESSIONS_DIR" -maxdepth 1 -name "cleanup_*.sql" 2>/dev/null | sort)
